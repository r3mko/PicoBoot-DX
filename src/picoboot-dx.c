...
// The payload must contain a big-endian header with the following layout:
//   [0] = magic 0 ("IPLB")
//   [1] = magic 1 ("OOT ")
//   [2] = size of payload in bytes (big-endian)
//   ...payload data...
//   [size - 1] = magic 2 ("PICO")
size_t validate_payload(void) {
    ...
    size_t size = raw_size_bytes / sizeof(p[0]);
    const size_t alignment = 1024 / sizeof(p[0]);
    const size_t size_aligned = (size + alignment - 1) / alignment * alignment;

    if (&p[size_aligned] > flash_payload_end) {
        return PAYLOAD_INVALID;
    }

    if (BigEndian32(p[size - 1]) != 0x5049434F) { // "PICO"
        return PAYLOAD_INVALID;
    }
...
void main() {
    ...
    if (payload_size != PAYLOAD_INVALID) {
        payload = flash_payload;
        channel_config_set_bswap(&c, true);
    } else {
        payload = ipl;
        payload_size = count_of(ipl);
    }
